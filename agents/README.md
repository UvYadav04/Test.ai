...
agent = TabularAgent([FileRef(file_id="f1", filename="f1.csv")], workspace_id="ws1")
...
agent = DocumentAgent([FileRef(file_id="handbook")])
findings = await agent.run("What is this document about?")
```

- `orchestrator/` - `OrchestratorAgent`, the top of the hierarchy: talks to the user, never runs SQL/RAG itself, delegates to fresh `TabularAgent`/`DocumentAgent` instances scoped to exactly the files it assigns them. Same tool-loop-then-formatter pattern, but returns `OrchestratorResult` (`final_answer`, `confidence`, `artifact_refs`, `open_questions`) instead of a `Findings` dataclass. A fresh `InvestigationState` (session_id, objective, selected_files, completed_tasks, findings, open_questions, status) is created per `run()` call - every `invoke_tabular_agent`/`invoke_document_agent`/`generate_hypotheses` call appends an event to it, and its `.summary()` feeds into the formatter alongside the transcript. `recall_user_info` (read-only) is separate from Investigation State - it reads facts that persist across sessions via `tools.orchestrator.memory.LongTermMemory` (a small JSON file, already precomputed into the task prompt at the start of every run - see `_context_brief`). There's no matching "save" tool: `worker_service/tasks/investigation.py`'s `update_chat_memory` arq job extracts and saves new preferences automatically after every completed turn, asynchronously, instead of the orchestrator spending a synchronous tool-call round trip on it mid-investigation. Pass a `storage` instance (e.g. `LocalParquetStore`) to enable the deliverable tools below - without it, `generate_csv`/`generate_report`/`generate_dashboard` raise instead of running.

  Deliverable tools (`tools.reporting.ReportingTools`, wrapped by `OrchestratorTools`): `generate_csv` and `generate_dashboard` both read an existing Parquet artifact via its `file_id` (from a Document Agent's `table_ref`, or from a Tabular Agent's `export_query` result) - never a path, see `analyzerEngine/sandbox/path_resolver.py` - and turn it into a `.csv` or a single-file HTML dashboard (Chart.js from CDN, no server needed). `generate_report` takes a short `context` string of findings the orchestrator already has and has an LLM compose it into a `.md` file (falling back to a deterministic template internally if composition fails - that fallback isn't a separate tool). If the objective needs freshly computed data exported (not something already sitting in a file), the orchestrator tells the Tabular Agent to call `export_query` instead of `query_data`, which persists the full result as a new Parquet file and returns its `file_id` - `query_data` alone only returns a row-capped preview, nothing is saved. Combining data across several files happens inside one `export_query` SQL call on the Tabular Agent (every assigned file is a queryable DuckDB view in the same connection), not by the orchestrator merging multiple agents' outputs itself. Every one of these three calls takes a `name` argument and creates `data/reports/<today's date>/<name>/` to hold the deliverable plus a copy of every source Parquet file it was built from, so each request's output is self-contained on disk.
